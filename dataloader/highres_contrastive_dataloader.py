...
class HighResContrastiveDataset(Dataset):
    """
    Dataset for high-resolution contrastive learning on 3D nuclei data.
    """
    def __init__(self, 
                 samples, 
                 target_size=(224, 224), 
...
def get_highres_contrastive_loaders(root_dir, 
                                    ...
                                    debug=False):
    """
    Get high-resolution contrastive dataloaders for training and validation.
    ...
    """
    print(f"Creating high-resolution contrastive dataloaders from {root_dir}")
    ...
    # Create datasets
    train_dataset = HighResContrastiveDataset(
        samples=train_samples,
        target_size=target_size,
        z_window_size=z_window_size
    )
    
    val_dataset = HighResContrastiveDataset(
        samples=val_samples,
        target_size=target_size,
        z_window_size=z_window_size
    )
