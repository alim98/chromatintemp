...
def default_transform(image, target_size=(80, 80)):
    """
    Default transform for 3D volume processing (with resizing).
...
class LowResImageDataset(Dataset):
    ...
    def __init__(self, 
        ...
        self.transform = transform if transform else (lambda x: default_transform(x, self.target_size))
        self.debug = debug
    ...
    def __getitem__(self, idx):
        ...
        try:
            ...
            try:
                ...
                # Fallback: match the expected shape for 3D or 2D
                if self.z_window_size > 1:
